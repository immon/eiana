...
import org.iana.rzm.web.common.validators.WebCountryCodeValidator;
...
public abstract class ContactEditor extends BaseComponent implements PageBeginRenderListener {
    ...
    @Component(id = "address",
               type = "TextArea",
               bindings = {"displayName=message:address-label", "value=ognl:contactAttributes.ADDRESS"})
    public abstract IComponent getStreetComponent();

    @Component(id = "email",
               type = "TextField",
               bindings = {"displayName=message:email-label", "value=ognl:contactAttributes.EMAIL", "validators=validators:email"})
    public abstract IComponent getEmailComponent();

    @Component(id = "privateEmail",
               type = "TextField",
               bindings = {"displayName=message:alt-email-label", "value=prop:privateEmail", "validators=validators:email"})
    public abstract IComponent getPrivateEmailComponent();

    @Component(id = "phone",
               type = "TextField",
               bindings = {"displayName=message:phone-label", "value=ognl:contactAttributes.PHONE"})
    public abstract IComponent getPhoneComponent();

    @Component(id = "altPhone",
               type = "TextField",
               bindings = {"displayName=message:alt-phone-label", "value=prop:alternatePhone"})
    public abstract IComponent getAltPhoneComponent();

    @Component(id = "fax",
               type = "TextField",
               bindings = {"displayName=message:fax-label", "value=ognl:contactAttributes.FAX"})
    public abstract IComponent getFaxComponent();

    @Component(id = "altFax",
               type = "TextField",
               bindings = {"displayName=message:alt-fax-label", "value=prop:alternateFax"})
    ...
    public  List<Value> getCountryNames(){
        return getRzmPage().getRzmServices().getCountrys();
    }
    ...
    public Value getCountry() {
        ...
        if (value == null) {
            return WebCountryCodeValidator.NO_COUNTRY;
        }
    ...
    public CountryCodeModel getCountryCodeModel() {
        ...
        countries.add(0, WebCountryCodeValidator.NO_COUNTRY);
    ...
    public void pageBeginRender(PageEvent event) {
        ...
        if (!event.getRequestCycle().isRewinding()) {
            ...
            if (StringUtil.isBlank(getPrivateEmail()) && (!isUsePrivateEmail())) {
                setPrivateEmail(getContactAttributes().get(ContactVOWrapper.PRIVATE_EMAIL));
                setUsePrivateEmail(StringUtil.isNotBlank(getPrivateEmail()));
            }

            if (StringUtil.isBlank(getAlternateFax()) && (!isAddAltFax())) {
                setAlternateFax(getContactAttributes().get(ContactVOWrapper.ALT_FAX));
                setAddAltFax(StringUtil.isNotBlank(getAlternateFax()));
            }

            if (StringUtil.isBlank(getAlternatePhone()) && (!isAddAltPhone())) {
                setAlternatePhone(getContactAttributes().get(ContactVOWrapper.ALT_PHONE));
                setAddAltPhone(StringUtil.isNotBlank(getAlternatePhone()));
            }
    ...
    public String getDisplayStyle(boolean visible) {
        if (visible) {
            return WebUtil.isIEBrowser(getRzmPage().getHttpRequest()) ? "display:inline" : "table-row";
        }
        return "display:none";
    }
    ...
    public void setContactName(String name) {
        ...
        if (!StringUtil.equals(oldValue, name)) {
    ...
    protected void validateCountryCode(String code) {
        if (code != null) {
            if (!getEditor().isValidCountryCode(code)) {
    ...
    public ProtectedPage getRzmPage(){
