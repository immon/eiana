...
class SubConfig implements Config {
    ...
    SubConfig(String subName, Config config) {
        ...
        if (subName.lastIndexOf(".") == subName.length() - 1)
            throw new IllegalArgumentException("subconfig name cannot contain \".\" on last position");
    ...
    private static String updateSubName(String subName, String paramName) {
        StringBuffer buff = new StringBuffer();
        if (paramName.contains(".")) {
            int lastDotIndex = paramName.lastIndexOf(".");
            buff.append(paramName.substring(0, lastDotIndex + 1));
            buff.append(subName).append(".");
            buff.append(paramName.substring(lastDotIndex + 1));
        } else {
            buff.append(subName).append(".");
