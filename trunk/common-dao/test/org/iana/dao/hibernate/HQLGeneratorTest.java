...
public class HQLGeneratorTest {
    ...
    public void testNoWhere() {
        HQLBuffer buf = new HQLGenerator().from(A.class, null);
        assert "from org.iana.dao.hibernate.A".equals(buf.getHQL().trim());
        assert buf.getParams().length == 0;
    }

    public void testEqual() {
        HQLBuffer buf = new HQLGenerator().from(A.class, new Equal("astr", "value"));
        assert "from org.iana.dao.hibernate.A where astr = ?".equals(buf.getHQL().trim());
        assert buf.getParams().length == 1;
    }

    public void testGreater() {
        HQLBuffer buf = new HQLGenerator().from(A.class, new Greater("astr", "value"));
        assert "from org.iana.dao.hibernate.A where astr > ?".equals(buf.getHQL().trim());
        assert buf.getParams().length == 1;
    }

    public void testGreaterEqual() {
        HQLBuffer buf = new HQLGenerator().from(A.class, new GreaterEqual("astr", "value"));
        assert "from org.iana.dao.hibernate.A where astr >= ?".equals(buf.getHQL().trim());
        assert buf.getParams().length == 1;
    }

    public void testLower() {
        HQLBuffer buf = new HQLGenerator().from(A.class, new Lower("astr", "value"));
        assert "from org.iana.dao.hibernate.A where astr < ?".equals(buf.getHQL().trim());
        assert buf.getParams().length == 1;
    }

    public void testLowerEqual() {
        HQLBuffer buf = new HQLGenerator().from(A.class, new LowerEqual("astr", "value"));
        assert "from org.iana.dao.hibernate.A where astr <= ?".equals(buf.getHQL().trim());
        assert buf.getParams().length == 1;
    }

    public void testLike() {
        HQLBuffer buf = new HQLGenerator().from(A.class, new Like("astr", "value"));
    ...
    public void testIn() {
        ...
        HQLBuffer buf = new HQLGenerator().from(A.class, new In("astr", values));
        assert "from org.iana.dao.hibernate.A where astr in (?,?)".equals(buf.getHQL().trim());
        assert buf.getParams().length == 2;
    }

    public void testIsNull() {
        HQLBuffer buf = new HQLGenerator().from(A.class, new IsNull("astr"));
        assert "from org.iana.dao.hibernate.A where astr is null".equals(buf.getHQL().trim());
        assert buf.getParams().length == 0;
    }

    public void testNot() {
        HQLBuffer buf = new HQLGenerator().from(A.class, new Not(new IsNull("astr")));
    ...
    public void testAnd() {
        ...
        HQLBuffer buf = new HQLGenerator().from(A.class, new And(args));
        System.out.println(buf.getHQL());
    ...
    public void testOr() {
        ...
        args.add(new IsNull("astr"));
        args.add(new Equal("atmsp", new Timestamp(System.currentTimeMillis())));
        HQLBuffer buf = new HQLGenerator().from(A.class, new Or(args));
    ...
    public void testSortSingleColumnAscNoWhere() {
        ...
        HQLBuffer buf = new HQLGenerator().from(A.class, crit);
        System.out.println(buf.getHQL());
